...
static int compare_versions(const char *latest_version, const char *running_version)
{
    ...
    if (latest_major > running_major) return 1;
    if (latest_major < running_major) return -1;

    if (latest_minor > running_minor) return 1;
    if (latest_minor < running_minor) return -1;

    if (latest_patch > running_patch) return 1;
    if (latest_patch < running_patch) return -1;
...
static void MQTT_handle_incoming_message(void)
{
    ...
    else if (incoming_message.msg_type == MSG_TYPE_CONFIG) // Assuming light_config is part of general config
    {
        ESP_LOGI(TAG, "Received CONFIG message with actionID: %d", incoming_message.action_id);
        // Handle config message based on action_id
        ...
        if (cJSON_IsNumber(system_type_json) && cJSON_IsArray(relay_names_array)) {
            ...
            if (cJSON_GetArraySize(relay_names_array) > 0 && cJSON_IsString(cJSON_GetArrayItem(relay_names_array, 0))) strncpy(name1, cJSON_GetArrayItem(relay_names_array, 0)->valuestring, sizeof(name1) - 1);
            if (cJSON_GetArraySize(relay_names_array) > 1 && cJSON_IsString(cJSON_GetArrayItem(relay_names_array, 1))) strncpy(name2, cJSON_GetArrayItem(relay_names_array, 1)->valuestring, sizeof(name2) - 1);
            if (cJSON_GetArraySize(relay_names_array) > 2 && cJSON_IsString(cJSON_GetArrayItem(relay_names_array, 2))) strncpy(name3, cJSON_GetArrayItem(relay_names_array, 2)->valuestring, sizeof(name3) - 1);
            
            LIGHT_CONTROL_set_system_config(new_system_type, name1, name2, name3);
            MQTT_send_ack_message(incoming_message.action_id);
            MQTT_send_relay_status_message(); // Send updated status
        } else {
...
static int MQTT_send_relay_status_message()
{
    ...
    if (my_mqtt.is_connected)
    {
        ...
        for (int i = 0; i < system_type; i++) { // Loop up to the configured system_type
